...
Experience the entire cryptographic pipeline. Prepare photon polarisations. Witness the collapse of superpositions. Finalise an unconditionally secure symmetric key, mathematically proven to withstand any adversary, ready for modern cipher suites like ChaCha20-Poly1305.
...
* **Mutually Unbiased Bases:** Master Rectilinear (+) and Diagonal (x) encoding schemes, visualising how orthogonal measurements force probabilistic outcomes.
...
* **Threat Modelling:** Unleash Eve. Toggle active interception to visualise devastating intercept-resend attacks in real time, observing how Eve's measurements inevitably collapse the wavefunction and induce errors.
* **Synchronised State Matrix:** Monitor the live data stream of Alice's prepared bits and bases against Bob's received measurements.
...
Security is not assumed; it is proven. Analyse the Quantum Bit Error Rate (QBER).
...
* **Information Reconciliation (Two Modes):** 
  ...
  * Utilise advanced **Low-Density Parity-Check (LDPC)** via Syndrome Decoding and Tanner graph belief propagation to eliminate discrepancies efficiently.
* **Privacy Amplification:** Eradicate any partial knowledge Eve may have gained during the sifting or error correction phases. Utilise **Universal Toeplitz Hashing** to mathematically compress the key, distilling out a perfectly secure final string.
...
* **Interactive Bisection UI:** Visualises the Cascade protocol block by block, demonstrating exactly how parity mismatch reveals single-bit errors.
...
Forged in the crucible of advanced academia. This simulator is a cornerstone project within the Master of Technology (M.Tech) in Quantum Computing program, specialising in Quantum Communication & Sensing.
